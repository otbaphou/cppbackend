...
namespace model
{
	...
	struct Item
	{
		...
		Item(Coordinates pos_, int id_, int type_)
		:pos(pos_),
		id(id_),
		type(type_)
		{}

		Item(Coordinates pos_, double width_, int id_, int type_)
			:pos(pos_),
			width(width_),
			id(id_),
			type(type_)
	...
	public:
		...
		void GenerateItems(unsigned int amount, const Data::MapExtras& extras)
		{
			...
			{
				int id = s + i;
				items_.push_back({GetRandomSpot(), id, GetRandomNumber(loot_table_.size())});
			}
		}
